<?php
    ...
    $query = "SELECT `admin`.id, `admin`.`name`, `admin`.`address`, `admin`.age,`admin`.phone, `admin`.`password`, `admin`.`image`, `admin`.email, `admin`.`role`, `role`.`description` AS roleDescription FROM `admin` JOIN `role` ON `admin`.`role`=`role`.id WHERE `admin`.email='$_SESSION[adminEmail]'";
    try{
    ...
    if(isset($_POST['updateAdmin'])){
        ...
        try{
            ...
            showMessage("An error occured while updating your data.");
        }
